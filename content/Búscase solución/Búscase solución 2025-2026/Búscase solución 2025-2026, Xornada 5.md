---
title: Búscase solución 2025-2026, xornada 5
description:
comments: true
draft: true
enableToc: true
tags:
  - Concurso
date: 2026-02-06
---

Benvido, cazador. Se aínda non estás rexistrado no **Gremio de Cazarrecompensas**, preme na seguinte **[ligazón](https://sementeira.maega.gal/B%C3%BAscase-soluci%C3%B3n/B%C3%BAscase-soluci%C3%B3n-2025-2026/B%C3%BAscase-soluci%C3%B3n-2025-2026)**. Se xa estás rexistrado, le as instrucións que veñen a continuación.

Para celebrar que retomamos a cacería, esta semana publicaremos o dobre de problemas, pero o habitual será que publiquemos un problema por categoría cada dúas semanas (os venres á 13:00).

Para enviar os problemas, facédelo dende o correo que puxéchedes á hora de inscribirvos ao [correo (mestrecazador@gmail.com)](mailto::mestrecazador@gmail.com) poñendo como asunto "Xornada5.NivelX.ProblemaX". Un correo por problema.

> [!solution2]- Nivel 1, Problema 1
> Atopa as solucións enteiras de $y^2=x^3+16$.

>[!solution2]-  Nivel 1, Problema 2
> Demostra que $F_{n+1}F_{n-1}-F_{n}^2=(-1)^{n}$, onde $F_{n}$ é o $n$-ésimo número de Fibonacci, $F_{0}=0$ e $F_{1}=1$.

>[!solution3]-  Nivel 2, Problema 1
 >Sexa $M\in\mathcal{M}_{2025\times2025}(\mathbb{R})$ tal que $|m_{i,j}|\leq 2^-(i+j)$ e que $m_{i,j}+m_{i+1,j}+m_{i,j+1}+m_{i+1,j+1}=0,\forall1\leq i,j\leq2024$. Proba que $\sum_{i,j=1}^{2025}m_{i,j}\leq \frac{1}{3}$.

>[!solution3]- Nivel 2, Problema 2
> Sexa $(\mathbb{S}^{1},+)$ a circunferencia unidade coa estrutura de grupo habitual. Atopa tódalas aplicacións continuas $f:\mathbb{S}^{1}\to\mathbb{S}^{1}$ tales que $f(x+y)=f(x)+f(y)$.
> 
> Nota: $(\mathbb{S}^{1},+)$ coa estrutura de grupo habitual pode entenderse de varias maneiras (todas equivalentes). Aquí explicitaremos dúas versións:
> - $\mathbb{S}^{1}=\{z\in\mathbb{C}/\lvert z\rvert=1\}$ coa operación $\cdot$ (produto).
> - $\mathbb{S}^{1}=\mathbb{R}/\sim$, con $x\sim y\iff x-y=2k\pi,k\in\mathbb{Z}$, coa operación $+$ habitual de $\mathbb{R}$ trasladada ao cociente.

>[!solution5]- Nivel 3, Problema 1
> Sexa $(X,\tau)$ un espazo topolóxico e $U\subset X$ un aberto conexo. Se $B\subset X$ é numerable, demostra que $U\setminus B$ é conexo por camiños.

>[!solution5]- Nivel 3, Problema 2
> Se $U$ é un aberto limitado de $\mathbb{R}^{n}$, consideramos $M$ como o supremo de $\{r>0 / \exists a\in U / B(a,r)\subset U\}$ e $R=\frac{M}{100}$.
> 
>  Agora xogamos un xogo entre dous xogadores en $U$, que consiste en colocar bólas de radio $R$ que estean contidas en $U$ e non intersequen ás anteriores. Gaña quen coloque a última bóla posible.
>  
>  Clasifica os abertos nos que gaña cada xogador se ambos xogan de forma óptima.

***"Que gane o mellor!"***

Se tedes algunha dúbida, podedes contactar á organización empregando o seguinte [correo (mestrecazador@gmail.com)](mailto::mestrecazador@gmail.com).
